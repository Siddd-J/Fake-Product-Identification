...
contract Production {
    ...
    mapping(bytes32 => bytes32) public productManufactured;
    ...
    // SELLER SECTION
    function addSeller(bytes32 _manufacturerId, bytes32 _sellerName, 
        bytes32 _sellerBrand, bytes32 _sellerCode, uint256 _sellerNum, 
        bytes32 _sellerManager, bytes32 _sellerAddress) public {
            
        sellers[sellerCount] = Seller(sellerCount, _sellerName, _sellerBrand, 
            _sellerCode, _sellerNum, _sellerManager, _sellerAddress);
        sellerCount++;
    ...
    function viewSellers() public view returns(
        ...
        uint256[] memory nums = new uint256[](sellerCount);
        bytes32[] memory managers = new bytes32[](sellerCount);
        bytes32[] memory addresses = new bytes32[](sellerCount);
        
        for(uint i = 0; i < sellerCount; i++) {
            ids[i] = sellers[i].sellerId;
            names[i] = sellers[i].sellerName;
            brands[i] = sellers[i].sellerBrand;
            codes[i] = sellers[i].sellerCode;
            nums[i] = sellers[i].sellerNum;
            managers[i] = sellers[i].sellerManager;
            addresses[i] = sellers[i].sellerAddress;
        }
        return (ids, names, brands, codes, nums, managers, addresses);
    }

    // PRODUCT SECTION
    function addProduct(bytes32 _manufacturerId, bytes32 _productSN, 
        bytes32 _productName, bytes32 _productBrand, 
        uint256 _productPrice) public {
            
        productItems[productCount] = ProductItem(productCount, _productSN, 
            _productName, _productBrand, _productPrice, "Available");
        productMap[_productSN] = productCount;
        productCount++;
        productsWithSeller[_manufacturerId].push(_productSN);
    }

    // ... (other functions would follow similarly)
 ...
 function viewProductItems() public view returns(
    ...
    function getPurchaseHistory(bytes32 _consumerCode) public view returns(
        ...
        for(uint i = 0; i < productSNs.length; i++) {
            sellerCodes[i] = productsSold[productSNs[i]];
            manufacturerCodes[i] = productManufactured[productSNs[i]];
        }
